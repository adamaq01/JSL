package org.jsl.bytecode;

import org.jsl.CodeDecoder;
import org.jsl.JSLException;
import org.jsl.fragments.*;
...
public class BytecodeDecoder extends CodeDecoder {
    ...
    @Override
    public List<CodeFragment> handleClass(Class<?> data) {
        List<CodeFragment> out = new ArrayList<>();
        try {
            if (data == null) return out;
            ClassReader reader = new ClassReader(data.getResourceAsStream(data.getSimpleName() + ".class"));
            ...
            for (FieldNode field : fieldNodes) {
                ...
                fieldFragment.name = field.name;
                fieldFragment.type = typesFromDesc(field.desc)[0];
            ...
            Collections.sort(methodNodes, (arg0, arg1) -> {
        ...
        for (int index = 0; index < instructions.size(); index++) {
            ...
            if (ainsnNode.getType() == AbstractInsnNode.INSN) {
                ...
                if (jumpNode.getOpcode() == IFEQ) {
                    ...
                    out.add(ifFrag);
                    toJump.push(jumpNode.label);
                } else if (jumpNode.getOpcode() == IFGE || jumpNode.getOpcode() == IFLE) {
                    IfStatementFragment ifFrag = new IfStatementFragment();
                    frames++;
                    ifs.push(jumpNode.label.getLabel());
                    ifFrag.toJump = jumpNode.label.getLabel().toString();
                ...
                if (methodNode.getOpcode() == INVOKESTATIC) {
                    ...
                    methodFragment.argumentsTypes = typesFromDesc(margs);
